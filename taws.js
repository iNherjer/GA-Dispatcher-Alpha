...
window.awmAnnounceWpAdvance = function(brng, distNM) {
    if (window.gaTrackerWarningsActive?.()) return;
...
window.awmStopLocalWarnings = function() {
    _awInterruptCurrentPlayback();
    _awQueue.length = 0;
};
...
function _awDrainQueue() {
    if (!_awmPlayOnThisDevice || window.gaTrackerWarningsActive?.()) {
        _awQueue.length = 0;
        _awQueueBusy = false;
        return;
    }
 ...
 */
const _awDetector = window.GANavigationWarnings.createAirspaceDetector();
function checkAirspaceWarnings(predPoints) {
    if (window.gaTrackerWarningsActive?.()) return;
    if (!_awmAirspaceWarn) return;
    if (!_awLoaded) { _awLoadClips(); return; }
    if (!_tawsAudioCtx || typeof activeAirspaces === 'undefined') return;
    for (const warning of _awDetector.evaluate({ airspaces: activeAirspaces, points: predPoints,
        gps: window.lastLiveGpsPos, lastTerrainFt: Number(window.lastLiveTerrainFt) || 0, readFreq: _awmReadFreq })) {
        const as = warning.as;
        ...
        _awPlaySequence(warning.clips); _awShowFreqBanner(as, col);
    }
}
...
async function checkTerrainAlongPath(points) {
    ...
    if (hasImmediateThreat && !window.gaTrackerWarningsActive?.()) {
