...
assert.match(appDeboarding, /_missionSceneBuildDeboardingCommand\(/);
const deboardingCommand = section(sync, 'function _missionSceneBuildDeboardingCommand(', 'function _missionSceneBuildDeboardingEffectCommand(');
assert.match(deboardingCommand, /_missionSceneMovingPersonTitle\(primaryGender, 'deboarding'\)/);
assert.match(deboardingCommand, /_missionSceneMovingPersonCandidates\(primaryGender, personTitle\)/);
...
assert.match(deboardingCommand, /if \(aptPickupPoint\) \{[\s\S]*commonFields\.vehicleArrival = false/);
