...
assert.ok(cargoSource.includes('<small>Gesamt</small>'));
assert.ok(cargoSource.includes('<small>Leer</small>'));
assert.ok(stylesSource.includes('grid-template-columns: repeat(4, minmax(0, 1fr))'));
assert.ok(stylesSource.includes('.mission-cargo-payload-metrics > span.is-half'));
