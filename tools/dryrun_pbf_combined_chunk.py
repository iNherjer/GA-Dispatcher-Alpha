...
def chunk_extract(con, pbf_path: str, bounds: Dict[str, float]) -> Dict[str, Any]:
    ...
    # Obs: wind + towers/masts (node based)
    obs_sql = f"""
    SELECT
      CASE
      ...
      CAST(COALESCE(TRY_CAST(REPLACE(tags['height'], ',', '.') AS DOUBLE), CASE WHEN tags['generator:source']='wind' THEN 120 ELSE 50 END) * 3.28084 AS BIGINT) AS hFt
    ...
    # Nodes in bbox for efficient way intersection + sampling
    # We keep broad infrastructure tags that are useful for routing/story/POI candidates.
    way_points_sql = f"""
    WITH
    bbox_nodes AS (
      SELECT id, lat, lon
    ...
    # Node POIs potentially useful as mission targets.
    poi_nodes_sql = f"""
    SELECT
    ...
    # Simple dedupe by rounded coordinate + class/name
    def dedupe(items: List[Dict[str, Any]], keys: List[str]):
        seen = set()
        out = []
        for it in items:
...
def main():
    ap = argparse.ArgumentParser(description="Dry-run: build one 25x25 NM combined tile from OSM PBF")
    ap.add_argument("--pbf", required=True, help="Path to .osm.pbf")
    ap.add_argument("--lat", type=float, default=None)
    ap.add_argument("--lon", type=float, default=None)
    ap.add_argument("--tile", default="", help="Optional explicit tile key latI|lonI")
