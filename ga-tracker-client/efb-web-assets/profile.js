...
function vpBuildStorageDiagnosticsLines() {
  ...
  var geoEntries = inventory.entries.filter(entry => entry.key.startsWith('ga_target_geo_context_v4_'));
  ...
  var missionStorage = window.gaMissionStorageDiagnostics;
...
window.vpBuildWeatherDebugReport = function () {
  ...
  if (voiceState) {
    ...
    if (recentVoiceLog.length) {
      var playedVoices = (window.gaPaxVoicePlaybackHistory || []).filter(x => x.epoch === voiceState.missionEpoch);
