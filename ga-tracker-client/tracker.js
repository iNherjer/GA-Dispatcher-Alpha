...
const TRACKER_VERSION = 'v401';
const TRACKER_VERSION_CODE = 401;
...
const TRACKER_FLIGHT_LOG_DIR = path.join(TRACKER_DATA_DIR, 'flightlogs');
...
const INPUT_EVENT_ENUM_REQUEST_ID = 9373;
...
function createMissionSmokeController(handle, getWs, syncId, pin, getLastGpsMsg = null, getGroundTrafficSnapshot = null, missionAuthority = null, missionShadow = null, onExecutionAck = null, onPayloadSnapshot = null) {
  ...
  const requestPayloadSnapshot = async (maxStations = 12) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      try { onPayloadSnapshot?.(snapshot); } catch (_) {}
      return snapshot;
    });
  };
  ...
  const reassertPa24PayloadSeats = async (rawState = {}, options = {}) => {
    const state = normalizePa24PayloadState(rawState);
    const applied = [];
    [2, 3, 4].forEach((seat) => {
      const character = Math.max(0, Math.min(4, Math.round(Number(state.seats[seat] || 0))));
      if (character <= 0) return;
      if (!setNamedVarValue(
        `L:Seat${seat}Character`,
        character,
        'enum',
        options.reason || 'pa24-payload-seat-reassert'
      )) {
        throw new Error(`pa24_seat_reassert_failed_${seat}`);
      }
  ...
  const missionPayloadHandler = createTrackerMissionPayloadHandler({
    ...
    recordRecovery: missionAuthority?.recordExecutionPayloadRecovery
      ? request => missionAuthority.recordExecutionPayloadRecovery(request)
      : null,
    getRecovery: missionAuthority?.getExecutionPayloadRecovery
      ? request => missionAuthority.getExecutionPayloadRecovery(request)
      : null,
    getPreviousRecovery: missionAuthority?.getLastExecutionPayloadRecovery
      ? request => missionAuthority.getLastExecutionPayloadRecovery(request)
      : null,
    sleep,
    log: debugLog
  });
  ...
  const isPickupVehicleObject = (obj) => {
    ...
    return /vehicle|fahrzeug|van|shuttle|bus|minibus|car|auto|truck|medic|fire/.test(text)
      && !/person|pax|passenger|cargo|box|kit|equipment/.test(text);
  };

  const isGroupPickupVehicleObject = (obj, groupPlan) => {
    if (!isPickupVehicleObject(obj) || groupPlan?.valid !== true) return false;
    const allowed = GROUP_VEHICLE_TITLES[groupPlan.groupVehicleKind] || [];
    const titles = [obj?.title, obj?.requestedTitle].map(value => String(value || '').trim().toLowerCase());
    return allowed.some(title => titles.includes(title.toLowerCase()));
  };
  ...
  const animateMissionSceneBoarding = async (command) => {
    ...
    if (groupPlan.enabled && !groupPlan.valid) {
      debugLog(`SCENE_BOARDING_ERROR scene=${sceneId} reason=${groupPlan.error}`);
      sendAck({ type: 'mission_scene_boarding_ack', commandId, sceneId, missionId, status: 'error', error: groupPlan.error, boarded: 0 });
      return;
    }
    const requestedBoarders = groupPlan.enabled
      ? groupPlan.expectedPassengerCount
      : clampInt(command?.boarderCount ?? command?.passengerCount ?? 1, 0, 3);
    ...
    const boarders = (groupPlan.enabled
      ? explicitBoarders
      : (explicitBoarders.length ? explicitBoarders : (fallbackPerson ? [fallbackPerson] : [])))
      .slice(0, requestedBoarders);
    if (groupPlan.enabled && boarders.length !== requestedBoarders) {
      debugLog(`SCENE_BOARDING_ERROR scene=${sceneId} reason=passenger_count_mismatch expected=${requestedBoarders} spawned=${boarders.length}`);
      sendAck({
        ...
        expectedPassengerCount: requestedBoarders,
        spawned: boarders.length,
        boarded: 0
      });
      return;
    }
    ...
    if (groupPlan.enabled && boarderPlans.length !== requestedBoarders) {
      debugLog(`SCENE_BOARDING_ERROR scene=${sceneId} reason=invalid_path expected=${requestedBoarders} planned=${boarderPlans.length}`);
      sendAck({
        type: 'mission_scene_boarding_ack',
        commandId,
        sceneId,
    ...
    const groupStaggerTotalMs = groupPlan.enabled
      ? Math.max(0, (requestedBoarders - 1) * groupPlan.boardingStaggerMs)
      : 0;
    const durationMs = clampInt(Math.max(requestedDurationMs, (distanceM / speedMps) * 1000 + cargoHoldMs + groupStaggerTotalMs + 8500), 3000, 45000);
    ...
    debugLog(`SCENE_BOARDING_START scene=${sceneId} boarders=${boarderPlans.length} expected=${requestedBoarders} group=${groupPlan.enabled ? 1 : 0} staggerMs=${groupPlan.enabled ? groupPlan.boardingStaggerMs : 0} objectIds=${boarderPlans.map(p => p.person.objectId).join(',')} path=${pathSource} durationMs=${durationMs} waypoints=${primaryPath.length - 1} speedKts=${speedKts} door=${doorEnabled ? 1 : 0} doorProfile=${doorProfile} aircraftSlot=${command?.aircraftSlot || ''} aircraftName="${command?.aircraftName || ''}" cargo=${cargoObjects.map(o => o.objectId).join(',') || 0} cargoPathIndex=${cargoPathIndex}`);
    ...
    const sendBoarderRoutes = async (routeForPlan) => {
      let sentCount = 0;
      for (let index = 0; index < boarderPlans.length; index++) {
        if (groupPlan.enabled && index > 0) await sleep(groupPlan.boardingStaggerMs);
        const plan = boarderPlans[index];
        const sent = sendWaypointRoute(plan.person.objectId, routeForPlan(plan), speedKts);
        sentCount += sent ? 1 : 0;
      }
      return sentCount;
    };
    if (canSplitAtCargo) {
      routeSentCount = await sendBoarderRoutes(plan => plan.path.slice(1, cargoPathIndex + 1));
      ...
      if (routeSent) {
        ...
        setTimeout(() => {
          ...
          boarderPlans.forEach((plan, index) => {
            ...
            }, groupPlan.enabled ? index * groupPlan.boardingStaggerMs : 0);
          });
        }, cargoDelayMs);
    ...
    if (!routeSent) {
      routeSentCount = await sendBoarderRoutes(plan => plan.path.slice(1));
      routeSent = routeSentCount > 0;
    ...
    if (!groupPlan.enabled && !routeSent && command?.fallbackTeleport === true) {
      ...
      await sleep(Math.max(500, durationMs - finalHoldMs - groupStaggerTotalMs));
    }
    ...
    const groupCompletion = evaluateGroupSequenceCompletion({
      expectedPassengerCount: requestedBoarders,
      spawnedCount: boarderPlans.length,
      routeSentCount
    });
    const boardingSucceeded = groupPlan.enabled ? groupCompletion.complete : routeSent;
    const boardedCount = boardingSucceeded ? (groupPlan.enabled ? requestedBoarders : boarderPlans.length) : 0;
    if (!groupPlan.enabled || boardingSucceeded) {
      sendAck({
    ...
    const vehicleDeparture = boardingSucceeded ? startVehicleDeparture(command, rec, sceneId) : false;
    const boardingError = boardingSucceeded ? '' : (groupPlan.enabled ? groupCompletion.error : 'waypoint_route_failed');
    debugLog(`SCENE_BOARDING_${boardingSucceeded ? 'OK' : 'ERROR'} scene=${sceneId} boarders=${boarderPlans.length} expected=${requestedBoarders} group=${groupPlan.enabled ? 1 : 0} routeSent=${boardingSucceeded ? 1 : 0} routeSentCount=${routeSentCount} removed=${removed} vehicleDeparture=${vehicleDeparture ? 1 : 0} error=${boardingError}`);
    sendAck({ type: 'mission_scene_boarding_ack', commandId, sceneId, missionId, status: boardingSucceeded ? 'ok' : 'error', routeSent: boardingSucceeded ? 1 : 0, routeSentCount, expectedPassengerCount: groupPlan.enabled ? requestedBoarders : undefined, removed, cargoRemoved, boarded: boardedCount, vehicleDeparture: vehicleDeparture ? 1 : 0, durationMs, error: boardingError });
    } finally {
  ...
  const animateMissionSceneDeboarding = async (command) => {
    ...
    const groupPlan = normalizeGroupSequenceCommand(command);
    if (groupPlan.enabled && !groupPlan.valid) {
      debugLog(`SCENE_DEBOARDING_ERROR scene=${sceneId} reason=${groupPlan.error}`);
      sendAck({ type: 'mission_scene_deboarding_ack', commandId, sceneId, missionId, status: 'error', error: groupPlan.error, deboarded: 0 });
      return;
    }
    const requestedBoarders = groupPlan.enabled
      ? groupPlan.expectedPassengerCount
    ...
    const pickupSceneRec = command?.deboardingPickupSceneId
      ...
      : null;
    const stagedPickupVehicle = (pickupSceneRec?.objects || []).find(obj => obj?.objectId && (
      groupPlan.enabled ? isGroupPickupVehicleObject(obj, groupPlan) : isPickupVehicleObject(obj)
    )) || null;
    ...
    trackerLog(`Scene ${sceneId}: Deboarding-Sequenz startet${vehicleArrivalEnabled ? ' mit Fahrzeug' : ' ohne Fahrzeug'} (${requestedBoarders} Pax${groupPlan.enabled ? ', Gruppe' : ''}).`);
    if (vehicleArrivalEnabled) {
      const vehicleStart = vehicleReturnRoute[0];
      const groupVehicle = groupPlan.enabled ? resolveGroupVehicleSelection(groupPlan, command) : null;
      const vehicleTitle = groupPlan.enabled
        ? groupVehicle.title
        : (String(command?.vehicleTitle || command?.vehicleObjectTitle || MISSION_SCENE_VEHICLE_TITLE).trim() || MISSION_SCENE_VEHICLE_TITLE);
      const vehiclePlan = {
        index: 1,
        kind: 'vehicle_return',
        label: 'Rueckkehr Fahrzeug',
        title: vehicleTitle,
        titleCandidates: groupPlan.enabled
          ? buildTitleCandidates(vehicleTitle, groupVehicle.candidates)
          : buildTitleCandidates(vehicleTitle, command?.vehicleTitleCandidates || ['Car Bush Firefighting', 'Car Bush Firefighting (FIREFIGHTING_DEFAULT)', 'FIREFIGHTING_DEFAULT', 'Car_Bush_Firefighting']),
    ...
    const groupPersonTitles = buildTitleCandidates(primaryPersonTitle, personTitles.length
      ? personTitles
      : [MISSION_SCENE_PERSON_TITLE, 'Tarmac_Male_Summer_Asian', 'Termac_Female_Summer_Asian', 'Termac_Male_Summer_Asian']);
    const personPlans = [];
    const start = reversePath[0];
    for (let i = 0; i < requestedBoarders; i++) {
      const baseStart = groupPlan.enabled
        ? { ...start }
        : { ...start, rightM: Number(start.rightM || 0) + (i * 0.8) };
      const absStart = relativeScenePoint(sceneBaseFromCommand(command, rec), baseStart, start);
      const personTitle = groupPlan.enabled
        ? groupPersonTitles[i % groupPersonTitles.length]
        : primaryPersonTitle;
      const personTitleCandidates = groupPlan.enabled
        ? groupPersonTitles.slice(i).concat(groupPersonTitles.slice(0, i))
        : groupPersonTitles;
      personPlans.push({
        index: i + 2,
        ...
        title: personTitle,
        titleCandidates: personTitleCandidates,
        lat: absStart.lat,
        lon: absStart.lon,
    ...
    if (!people.length || (groupPlan.enabled && people.length !== requestedBoarders)) {
      ...
      people.forEach(person => removeSceneObject(rec, person, 'deboarding-partial-spawn-cleanup'));
      if (arrivalVehicle) removeSceneObject(rec, arrivalVehicle, 'deboarding-partial-spawn-vehicle-cleanup');
      const spawnError = groupPlan.enabled ? 'passenger_count_mismatch' : 'person_spawn_failed';
      sendAck({
        type: 'mission_scene_deboarding_ack',
        commandId,
        sceneId,
    ...
    const deboardingGroupStaggerMs = groupPlan.enabled
      ? Math.max(0, (requestedBoarders - 1) * groupPlan.boardingStaggerMs)
      : 0;
    ...
    const groupCompletion = evaluateGroupSequenceCompletion({
      expectedPassengerCount: requestedBoarders,
      spawnedCount: people.length,
      routeSentCount
    });
    const allPersonRoutesSent = groupPlan.enabled
      ? groupCompletion.complete
      : routeSentCount === people.length;
    ...
    if (boardedPickup && (!groupPlan.enabled || allPersonRoutesSent)) {
  ...
  const observeMissionShadow = (command = {}, result = null) => {
    if (!missionShadow) return null;
    const activeRun = missionAuthority?.getActiveRun?.({ includeBundle: true }) || null;
    try {
      return missionShadow.observeAuthorityResult(
        result,
        activeRun,
        command?.reason || command?.type || 'authority-update'
      );
    } catch (error) {
      debugLog(`MISSION_SHADOW_ERROR mission=${activeRun?.missionId || ''} run=${activeRun?.runId || ''} error=${error?.message || error}`);
      return null;
    }
  };
          ...
          if (result?.ok && command?.resumeBundle && missionShadow) {
            try {
              missionShadow.observe({
                ...
                runId: command.runId,
                authorityRevision: result.releasedRun?.revision || 0,
                resumeBundle: command.resumeBundle,
                source: command.reason || 'mission-authority-release'
              });
            } catch (error) {
  ...
  return {
    async syncExecutionPayload(request = {}) {
      const activeRun = missionAuthority?.getActiveRun?.() || null;
      const missionId = String(request?.missionId || '').trim();
      const runId = String(request?.runId || '').trim();
      if (!activeRun?.missionId || !activeRun?.runId) {
        return { ok: false, status: 'conflict', error: 'no_active_run', sideEffect: false };
      }
      ...
      return missionPayloadHandler.syncBeforeStart(request);
    },
    async scheduleExecutionPayload(request = {}) {
      const activeRun = missionAuthority?.getActiveRun?.() || null;
      const missionId = String(request?.missionId || '').trim();
      const runId = String(request?.runId || '').trim();
      if (!activeRun?.missionId || !activeRun?.runId) {
      ...
      return missionPayloadHandler.scheduleManifestSync(request);
    },
    cancelExecutionPayloadSync(reason = 'simulator-detached') {
      return missionPayloadHandler.cancelPayloadSyncQueue(reason);
    },
    async cleanupExecutionRun(request = {}) {
      const activeRun = missionAuthority?.getActiveRun?.() || null;
      const missionId = String(request?.missionId || '').trim();
      const runId = String(request?.runId || '').trim();
      if (!activeRun?.missionId || !activeRun?.runId) {
      ...
      if (missionId !== activeRun.missionId || runId !== activeRun.runId) {
        return { ok: false, status: 'conflict', error: 'mission_run_conflict', cleared: 0, sideEffect: false };
      }
      try {
        const executionSnapshot = missionAuthority?.getExecutionSnapshot?.() || null;
        const payloadRestore = await missionPayloadHandler.restoreForAbort({
          ...request,
          manifest: executionSnapshot?.state?.manifest || null
        });
        if (!payloadRestore?.ok) {
          return {
            ok: false,
            status: payloadRestore?.status || 'error',
            error: payloadRestore?.error || 'mission_payload_restore_failed',
            cleared: 0,
            sideEffect: payloadRestore?.sideEffect === true,
            payloadRestore
          };
        }
        const cleared = await clearAuthorityRunEffects(missionId, request?.reason || 'mission-execution-abort');
        return {
          ok: true,
          status: cleared > 0 || payloadRestore?.sideEffect === true ? 'ok' : 'noop',
          cleared,
          sideEffect: cleared > 0 || payloadRestore?.sideEffect === true,
          payloadRestore
        };
      } catch (error) {
    ...
    dispatchExecutionCommand(command) {
      ...
      if (type === 'mission_scene_deboarding_continue') {
        const sceneId = String(command?.sceneId || '').trim();
        const deboardingCommandId = String(command?.deboardingCommandId || '').trim();
        if (!sceneId || !deboardingCommandId) {
          return { ok: false, status: 'blocked', error: 'mission_deboarding_continuation_target_missing', sideEffect: false };
        }
        debugLog(`MISSION_EFFECT_DISPATCH type=${type} commandId=${commandId} scene=${sceneId} deboardingCommandId=${deboardingCommandId}`);
        const continued = resolveDeboardingContinuation(command, 'continue');
        return {
          ok: true,
          status: 'completed',
          sideEffect: continued === true,
          duplicate: continued !== true,
          commandId
        };
      }
    ...
    refreshPayloadSnapshot(maxStations = 12) {
      return requestPayloadSnapshot(maxStations);
    },
...
function startTracker(syncId, pin, voiceCredentials = null) {
  ...
  if (TRACKER_APT_EXECUTION_BLOCK_REASON) {
    ...
    trackerLog('APT-Tracker-Ausführung bleibt bis zum vollständigen App-Paritätsnachweis im Legacy-/Shadow-Modus.');
  }
  ...
  const trackerVoiceState = trackerVoiceService.publicState();
  trackerLog(trackerVoiceState.configured
    ? `🔊 Zentrale Voice-Ausgabe bereit (${trackerVoiceState.provider}).`
    : '🔇 Zentrale Voice-Ausgabe nicht konfiguriert; Browser-Fallback bleibt aktiv.');
  let trackerCockpitControl = null;
  ...
  if (recoveredMissionRun?.resumeBundle) {
    trackerMissionShadow.observe({
      missionId: recoveredMissionRun.missionId,
      runId: recoveredMissionRun.runId,
      authorityRevision: recoveredMissionRun.revision,
      resumeBundle: recoveredMissionRun.resumeBundle,
      source: 'tracker-recovery'
    });
  }
  ...
  const refreshCloudMissionCandidate = async (reason = 'interval') => {
    ...
    try {
      ...
      debugLog(`MISSION_CLOUD_SYNC_ERROR reason=${reason} code=exception error=${error?.message || error}`);
      return _cloudMissionCandidate;
    } finally {
  ...
  const activateCloudMission = async (request = {}) => {
    ...
    const acquired = missionAuthorityManager.acquire({
      ...
      reason: 'efb-cloud-mission-activation',
      state: 'active',
    ...
    const prepared = missionAuthorityManager.prepareExecutionAuthority({
      ...
      runId: active.runId,
      clientId: ownerClientId,
      commandId: `${request.commandId}:handoff-prepare`,
      reason: 'efb-cloud-mission-activation',
    ...
    if (!prepared.ok) {
      missionAuthorityManager.release({
        ...
        runId: active.runId,
        clientId: ownerClientId,
        commandId: `${request.commandId}:rollback-release`,
        reason: prepared.error || 'cloud-handoff-prepare-failed'
      });
    ...
    const committed = missionAuthorityManager.commitExecutionAuthority({
      ...
      runId: active.runId,
      clientId: ownerClientId,
      commandId: `${request.commandId}:handoff-commit`,
      reason: 'efb-cloud-mission-activation',
      expectedRevision: active.revision,
    ...
    if (!committed.ok) {
      missionAuthorityManager.release({
        ...
        runId: active.runId,
        clientId: ownerClientId,
        commandId: `${request.commandId}:commit-release`,
        reason: committed.error || 'cloud-handoff-commit-failed'
      });
      return { ...committed, sideEffect: false };
    }
    active = missionAuthorityManager.getActiveRun({ includeBundle: true });
    trackerMissionShadow.observe({
      missionId: active.missionId,
      runId: active.runId,
  ...
  trackerCockpitControl = createTrackerCockpitControl({
    ...
    executeIntent: async request => {
      ...
      if (result?.ok === true && String(request?.intent || request?.action || '').toLowerCase() === 'abort_mission') {
        ...
        // before the planned mission can be offered again.
        await refreshCloudMissionCandidate('cockpit-abort');
      }
      broadcastMissionAuthorityUpdate('cockpit-intent', request, result);
      return result;
    },
    activateMission: async request => {
      const result = await activateCloudMission(request);
      broadcastMissionAuthorityUpdate('cockpit-cloud-activation', request, result);
      return result;
    }
  });
  ...
  try {
    ...
    _efbHttpServer = createTrackerEfbHttpServer({
      ...
      getStatus: () => ({
        ...
        missionAvailable: Boolean(missionAuthorityManager.getActiveRun() || (TRACKER_APT_EXECUTION_ENABLED && _cloudMissionCandidate)),
        lastMissionSnapshotAt: Number(missionAuthorityManager.getActiveRun()?.updatedAt || (TRACKER_APT_EXECUTION_ENABLED ? _cloudMissionCandidate?.updatedAt : 0)) || null,
        cloudMissionStatus: _cloudMissionLastStatus,
        cloudMissionAvailable: Boolean(TRACKER_APT_EXECUTION_ENABLED && _cloudMissionCandidate),
        cloudMissionLastAttemptAt: _cloudMissionLastAttemptAt || null,
        cloudMissionLastSuccessAt: _cloudMissionLastSuccessAt || null,
        missionShadow: trackerMissionShadow.publicState(),
        missionExecution: missionExecutionRuntime.publicState(),
        flightLog: trackerFlightLogStore.publicState(),
        aptMissionTest: {
          enabled: true,
          automatic: true,
          scope: 'all',
      ...
      getMissionSnapshot: () => {
        ...
        if (TRACKER_APT_EXECUTION_ENABLED && _cloudMissionCandidate) {
          const cloudProjected = projectTrackerEfbMissionView({
            missionId: _cloudMissionCandidate.missionId,
            runId: _cloudMissionCandidate.runId,
            authority: 'tracker',
          ...
          if (cloudProjected) return {
            ...
            cloudPending: true,
            authoritySnapshot,
            control: _cloudMissionCandidate.control,
            executionShadow: trackerMissionShadow.publicState()
          };
        }
  ...
  broadcastMissionAuthorityUpdate = (reason = 'mission-update', request = {}, result = {}) => {
    ...
    try {
      relayFanout.send(JSON.stringify({
        type: 'gps',
        syncId,
        pin,
        ...
        missionUpdate: {
          ...
          status: String(result?.status || (result?.ok ? 'ok' : 'error')).slice(0, 40)
        },
        sentAt: Date.now()
      }));
      return true;
    } catch (error) {
      debugLog(`MISSION_AUTHORITY_BROADCAST_ERROR reason=${reason} error=${error?.message || error}`);
      return false;
    }
  };
  ...
  const handleTrackerMessage = (raw) => {
    ...
    if (!isTrustedMissionIntentEnvelope(data, command, pin)) {
      debugLog(`RELAY_COMMAND_REJECT type=${commandType} commandId=${commandId || 'none'} reason=mission-intent-pin-required`);
      return;
    }
    if (trackerCommand) {
  ...
  const missionCloudStartTimer = TRACKER_APT_EXECUTION_ENABLED
    ? setTimeout(() => refreshCloudMissionCandidate('startup'), 800)
    : null;
  ...
  if (typeof missionCloudStartTimer?.unref === 'function') missionCloudStartTimer.unref();
  if (typeof missionCloudInterval?.unref === 'function') missionCloudInterval.unref();
  startSimConnectOnce();
  ...
  open('VFR-Multitool-v206', 5)
    .then(({ handle }) => {
      ...
      const refreshMissionPayloadSnapshot = () => {
        ...
        if (!execution || execution.executionAuthority !== 'tracker' || execution.recipe !== 'apt' || payloadSnapshotRefreshPending) return;
        payloadSnapshotRefreshPending = true;
        missionSmokeController.refreshPayloadSnapshot(12)
          .catch(() => null)
          .finally(() => { payloadSnapshotRefreshPending = false; });
      };
      const payloadSnapshotInterval = setInterval(refreshMissionPayloadSnapshot, 5000);
      refreshMissionPayloadSnapshot();
      missionSimulatorEffects = missionExecutionRuntime?.attachSimulator?.({
        ...
        syncPayloadBeforeStart: request => missionSmokeController.syncExecutionPayload(request),
        syncPayloadManifestState: request => missionSmokeController.scheduleExecutionPayload(request),
        cancelPayloadSync: reason => missionSmokeController.cancelExecutionPayloadSync(reason),
        cleanupMission: request => missionSmokeController.cleanupExecutionRun(request)
      }) || null;
      const sendHomebaseAck = (payload = {}) => {
        const ws = getWs();
        if (!ws || ws.readyState !== WebSocket.OPEN) return;
      ...
      const sendMissionIntentAck = (command = {}, result = {}) => {
        ...
        try {
          const msg = {
            ...
            trackerVersionCode: TRACKER_VERSION_CODE,
            commandAckOnly: true,
            trackerMissionAuthority: authoritySnapshot,
            trackerAck: {
              source: 'tracker',
              type: 'mission_execution_intent_ack',
              commandId: command?.commandId || null,
              missionId: command?.missionId || result?.activeRun?.missionId || result?.releasedRun?.missionId || '',
              runId: command?.runId || result?.activeRun?.runId || result?.releasedRun?.runId || '',
              intent: command?.intent || '',
              ok: result?.ok === true,
              status: result?.status || (result?.ok ? 'ok' : 'error'),
              error: result?.error || '',
              sideEffect: result?.sideEffect === true,
              activeRun: result?.activeRun || authoritySnapshot?.activeRun || null,
              releasedRun: result?.releasedRun || null,
          ...
          ws.send(JSON.stringify(msg));
          debugLog(`MISSION_RELAY_INTENT_ACK intent=${command?.intent || 'unknown'} status=${msg.trackerAck.status} commandId=${command?.commandId || ''} requestRun=${command?.runId || 'none'} activeRun=${msg.trackerAck.activeRun?.runId || 'none'} revision=${msg.trackerAck.activeRun?.revision || 0} error=${msg.trackerAck.error || 'none'}`);
          return true;
        } catch (error) {
          debugLog(`MISSION_RELAY_INTENT_ACK_ERROR commandId=${command?.commandId || ''} error=${error?.message || error}`);
          return false;
        }
      };
      ...
      if (typeof setTrackerCommandHandler === 'function') {
        setTrackerCommandHandler((command) => {
          const type = String(command?.type || '');
          if (type === 'mission_execution_intent') {
            const execute = TRACKER_APT_EXECUTION_ENABLED
              ...
              : Promise.resolve({
                  ...
                  sideEffect: false,
                  activeRun: missionAuthorityManager?.getActiveRun?.() || null
                });
            execute.then(result => {
              if (typeof updateEfbState === 'function') {
                updateEfbState({ missionSnapshot: missionSmokeController.getTrackerMissionStatus() });
              }
              sendMissionIntentAck(command, result);
            }).catch(error => {
              sendMissionIntentAck(command, {
                ok: false,
                status: 'error',
              ...
              if (validPosition) {
                ...
                if (missionExecutionRuntime?.enabled
                  ...
                  const executionTelemetry = missionExecutionRuntime.observeTelemetry({
                    ...
                    simPaused,
                    simPausedA: Number.isFinite(Number(simPausedA)) ? Number(simPausedA) : null,
                    simPausedB: Number.isFinite(Number(simPausedB)) ? Number(simPausedB) : null,
                    pauseFlags: runtimeState.pauseFlags || 0,
...
async function verifyAndStartTracker(syncId, pin, { promptOnFailure = false, voiceCredentials = null } = {}) {
...
function main() {
  ...
  if (HEADLESS_MODE) {
    ...
    rl.once('line', (line) => {
      ...
      const voiceProvider = String(credentials?.voice?.provider || '').trim().toLowerCase();
      const voiceApiKey = String(credentials?.voice?.apiKey || '').trim();
      const voiceCredentials = voiceApiKey && ['gemini', 'openai'].includes(voiceProvider)
        ? { provider: voiceProvider, apiKey: voiceApiKey }
        : null;
      ...
      if (voiceCredentials) trackerLog(`🔒 Voice-Zugang für ${voiceProvider} wurde geschützt in den Tracker-Speicher übernommen.`);
      verifyAndStartTracker(syncId, pin, { promptOnFailure: false, voiceCredentials });
    });
    return;
  }
  savedId = data.syncId || '';
