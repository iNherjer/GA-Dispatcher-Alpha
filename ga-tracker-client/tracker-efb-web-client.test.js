...
test('tracker-hosted EFB page uses the original Kartentisch DOM and shared app modules', () => {
  ...
  assert.equal(EFB_WEB_ASSET_REVISION, '40001');
  assert.match(page, /data-efb-view-version="9"/);
  assert.match(page, /app-styles\.css\?v=40001/);
  assert.match(page, /host\.css\?v=40001/);
  assert.match(page, /map-shell-core\.js\?v=40001/);
  assert.match(page, /map-utility-tools\.js\?v=40001/);
  assert.match(page, /mission-control-ui-core\.js\?v=40001/);
  assert.match(page, /cockpit-session-client\.js\?v=40001/);
  assert.match(page, /host\.js\?v=40001/);
  ...
  assert.match(page, /src="\/efb\/v1\/assets\/map-utility-tools\.js\?v=40001"/);
  assert.match(page, /src="\/efb\/v1\/assets\/mission-control-ui-core\.js\?v=40001"/);
  assert.match(page, /src="\/efb\/v1\/assets\/cockpit-session-client\.js\?v=40001"/);
  assert.match(page, /src="\/efb\/v1\/assets\/host\.js\?v=40001"/);
  ...
  const scriptOrder = [
    ...
    '/efb/v1/assets/map-shell-core.js?v=40001',
    '/efb/v1/assets/map-utility-tools.js?v=40001',
    '/efb/v1/assets/mission-control-ui-core.js?v=40001',
    '/efb/v1/assets/cockpit-session-client.js?v=40001',
    '/efb/v1/assets/host.js?v=40001'
...
test('Kartentisch markup extraction stays bounded to the map and rewrites E6B locally', () => {
  ...
  assert.match(markup, /id="mapE6BFlip"[^>]*>↻<\/button>/);
  assert.match(markup, /id="mapE6BClose"[^>]*>×<\/button>/);
...
test('unchanged mission polls do not redraw the EFB banner, toolbar or cargo overlay', () => {
  const source = getTrackerEfbWebClientAsset('/efb/v1/assets/host.js').body.toString('utf8');
  assert.match(source, /var missionPresentationSignature = ''/);
  const renderStart = source.indexOf('function renderMissionPayload(payload)');
  const presentationGuard = source.indexOf('if (presentationSignature !== missionPresentationSignature)', renderStart);
  const bannerRender = source.indexOf('renderMissionActionBanner(next);', renderStart);
  const toolbarRender = source.indexOf('renderMissionToolbar(next);', renderStart);
  const cargoRender = source.indexOf('renderCargoManager();', renderStart);
  assert.ok(renderStart >= 0 && presentationGuard > renderStart);
  assert.ok(bannerRender > presentationGuard);
  assert.ok(toolbarRender > presentationGuard);
  assert.ok(cargoRender > presentationGuard);
});
...
test('EFB map banner exposes only contextual tracker-approved actions', () => {
  const hostSource = getTrackerEfbWebClientAsset('/efb/v1/assets/host.js').body.toString('utf8');
  const modelSource = hostSource.match(/function missionActionBannerModel\(payload\) \{[\s\S]*?\n  \}\n\n  function setupMissionActionBanner/)?.[0]
    .replace(/\n\n  function setupMissionActionBanner$/, '');
  assert.ok(modelSource);
  const missionActionBannerModel = new Function(`return (${modelSource});`)();

  assert.equal(missionActionBannerModel(null), null);
  const readOnly = missionActionBannerModel({
    available: true,
    missionId: 'mission-1',
    phase: 'planned',
  ...
  const prepare = missionActionBannerModel({
    available: true,
    missionId: 'mission-1',
    phase: 'planned',
    view: { title: 'Kurzer Sprung', currentTask: 'Mission vorbereiten' },
    control: { executionAuthority: 'tracker', authorityRevision: 4, phase: 'planned', allowedActions: ['prepare_mission'] }
  });
  assert.equal(prepare.kicker, 'Mission bereit');
  assert.equal(prepare.intent, 'prepare_mission');
  assert.equal(prepare.kind, 'intent');

  const cargo = missionActionBannerModel({
    available: true,
    missionId: 'mission-1',
    view: { currentTask: 'Abflugmanifest vervollständigen' },
    control: { executionAuthority: 'tracker', authorityRevision: 5, phase: 'boarding', allowedActions: ['set_manifest_item'] }
  });
  assert.equal(cargo.kind, 'cargo');
  assert.equal(cargo.button, 'Verladung öffnen');

  const enroute = missionActionBannerModel({
    available: true,
    missionId: 'mission-1',
...
test('EFB packages the warning decoder and voice catalog without the standalone App', () => {
  const page = createTrackerEfbWebClientPage();
  assert.ok(page.indexOf('/assets/navigation-warning-audio.js') < page.indexOf('/assets/audio-client.js'));
