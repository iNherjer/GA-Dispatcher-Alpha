...
const { missionId, bundle } = require('./tracker-mission-apt-training-fixture.js');
...
test('APT training uses common loading, ready intent, stale revision protection and abort in live authority', async t => {
  ...
  assert.ok(h.manager.getExecutionSnapshot().state.trainingTask.state.progress.startAvailable);
  assert.ok(h.manager.getExecutionSnapshot().view.allowedActions.includes('training_ready'));
  const beforeRepeat = structuredClone(h.manager.getExecutionSnapshot().state.trainingTask.state.checkpoint);
  assert.equal((await h.intent('training_repeat_instruction')).ok, true);
  assert.deepEqual(h.manager.getExecutionSnapshot().state.trainingTask.state.checkpoint, beforeRepeat);
  ...
  assert.equal(h.manager.getExecutionSnapshot().state.trainingTask.state.progress.activeExercise.status, 'active');
  assert.equal((await h.intent('training_abort')).ok, true);
  assert.equal(h.manager.getExecutionSnapshot().state.trainingTask.state.progress.activeExercise.status, 'repeat');
...
test('APT training telemetry and intents run in the real mission child process', async t => {
  ...
  assert.equal(snapshot.state.trainingTask.state.progress.startAvailable, true);
  ...
  snapshot = host.authorityManager.getExecutionSnapshot();
  assert.equal(snapshot.state.trainingTask.state.progress.activeExercise.status, 'repeat');
});
...
test('APT completion and optional exercises preserve destination lifecycle; telemetry gaps preserve completed work', async t => {
  ...
  assert.notEqual(state.flags.targetSatisfied,true);
  assert.equal((await h.intent('training_extra')).ok,true);
  for(let i=0;i<4;i++)await fly(180);
  assert.equal((await h.intent('training_ready')).ok,true);
  await fly(180);await fly(190,30,7000);
  state=h.manager.getExecutionSnapshot().state;
  assert.equal(state.trainingTask.state.progress.completedCount,1);
  assert.equal(state.trainingTask.state.progress.activeExercise.status,'repeat');
  assert.ok(state.trainingTask.state.guidance);
  assert.deepEqual(execution.normalizeState(JSON.parse(JSON.stringify(state))).trainingTask,state.trainingTask);
});
