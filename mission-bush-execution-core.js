(function(root, factory) {
...
})(typeof globalThis !== 'undefined' ? globalThis : this, function(boardingCore, farewellCore) {
    'use strict';
    const SCHEMA = 'ga.mission-bush-execution-recipe.v1';
    const CAPABILITY = 'mission.bush-strip.v1';
    const PROFILES = Object.freeze({bush_supply_strip:'unload_at_target', bush_charter_strip:'passenger_dropoff', bush_scenic_hopper:'land_at_target'});
    ...
    function validateSpec(spec) {
        ...
        if (spec.targetMode !== 'strip' || spec.completionMode !== PROFILES[spec.profileId]
            || spec.requiresReturnHome !== false || spec.pickupKind
            || !same(spec.allowedEndLocations,['target']) || (spec.recipeId && spec.recipeId !== 'strip_target')) return 'bush_strip_contract_invalid';
        if (!point(spec.targetRef) || !point(spec.homeRef)) return 'bush_strip_location_invalid';
        return null;
    }
    function validateRecipe(recipe) {
        if (!recipe || recipe.schema!==SCHEMA || recipe.version!==1 || recipe.kind!=='strip_target'
    ...
    function validateBundle(bundle) {
        ...
        if (contracts.some(c=>c.sarHeli || c.trainingProcedure || c.surveyPattern || c.poiChain)
            || bundle.executionPoiRecipe || bundle.executionTrainingRecipe) return 'bush_recipe_specialization_not_migrated';
        ...
        if (plan.effects?.['voice.farewell']?.context?.supported!==true
            || !plan.effects?.['voice.boarding']?.recipe) return 'bush_voice_context_missing';
        ...
        if (!boardingCore?.normalizeRecipe(boarding) || !farewellCore?.normalizeContext(farewell)
            || [boarding,farewell].some(v=>v.missionId!==bundle.missionId)
