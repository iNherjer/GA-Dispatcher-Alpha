...
window.vpBuildWeatherDebugReport = function() {
    ...
    lines.push(`- Tracker: connected=${window.liveTrackerConnected ? '1' : '0'} | version=${window.liveTrackerVersionLabel || '-'} | code=${Number.isFinite(Number(window.liveTrackerVersionCode)) ? Number(window.liveTrackerVersionCode) : '-'} | missionUi=${window.GAMissionControlUiCore?.BUILD_ID || '-'} | relay=${window.liveGpsRelayCode || '-'} | telemetry=${window.liveTrackerTelemetryMode || 'unknown'}${window.liveTrackerTelemetryReason ? `:${window.liveTrackerTelemetryReason}` : ''} | since=${vpFormatDebugTs(window.liveTrackerTelemetrySince)}`);
    const hibPos = window.liveTrackerLastKnownPosition;
    const wakeReq = window.gaLastTrackerWakeRequest;
    const wakeAck = window.gaLastTrackerWakeAck;
    lines.push(`- Tracker HIB/Wake: lastPos=${Number.isFinite(Number(hibPos?.lat)) && Number.isFinite(Number(hibPos?.lon)) ? `${Number(hibPos.lat).toFixed(5)},${Number(hibPos.lon).toFixed(5)}@${vpFormatDebugTs(hibPos.t)}` : '-'} | wakeReq=${wakeReq?.reason || '-'}@${vpFormatDebugTs(wakeReq?.at)} | wakeAck=${wakeAck?.status || '-'}${wakeAck?.blockedReason ? `:${wakeAck.blockedReason}` : ''}@${vpFormatDebugTs(wakeAck?.receivedAt)}`);
