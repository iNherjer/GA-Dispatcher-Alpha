// Generated from mission-training-procedure.js by tools/generate-training-core.mjs.
...
clock = clock && typeof clock.now === 'function' ? clock : { now: () => Date.now() };
    ...
    function sampleFromInput(input = {}) {
        ...
        return {
            ...
            nowMs: finiteNumber(input.nowMs ?? clock.now(), clock.now())
        };
    }
    ...
    function pushGateEvent(state, events, kind, sample, extra = {}) {
        ...
        const now = Number(sample?.nowMs || clock.now());
    ...
    function tickState(recipeRaw, stateRaw, sampleRaw) {
        ...
        if (sample.onGround || !Number.isFinite(sample.altFt) || !Number.isFinite(sample.headingDeg)) {
            state.updatedAt = Number(sample.nowMs || clock.now());
    ...
    function abortExercise(missionData = null, passenger = null) {
        ...
        resetExerciseStartGate(ctx.state, clock.now(), 5000);
        ctx.state.updatedAt = clock.now();
        return { ok: true, state: snapshotState(ctx.state), recipe: ctx.recipe };
    }
