...
// Shared follow/autozoom implementation: map-autozoom.js.
...
const MISSION_INTENT_CAPABILITY = 'mission.intent.v1';
...
function _trackerSupportsMissionIntents() {
    return Array.isArray(window.liveTrackerCapabilities)
        && window.liveTrackerCapabilities.includes(MISSION_INTENT_CAPABILITY);
}
window.trackerSupportsMissionIntents = _trackerSupportsMissionIntents;

function _missionExecutionControlSnapshot() {
    const authority = window.lastTrackerMissionAuthority;
    return authority?.execution && typeof authority.execution === 'object'
        ? authority.execution
        : null;
}

function _missionExecutionAuthorityIsTracker() {
    const active = window.lastTrackerMissionAuthority?.activeRun || window.lastTrackerMissionStatus || null;
    return active?.executionAuthority === 'tracker'
        || _missionExecutionControlSnapshot()?.executionAuthority === 'tracker'
        || window.gaTrackerExecutionControl?.executionAuthority === 'tracker';
}
window.gaTrackerExecutionHandlesMission = _missionExecutionAuthorityIsTracker;
...
function _buildMissionAuthorityMapProfile() {
    ...
    return {
        ...
        obstacles,
        airspaces,
        context: { ...context, theme: localStorage.getItem('ga_theme') || 'classic',
            departureIcao: typeof currentStartICAO !== 'undefined' ? currentStartICAO : '',
            destinationIcao: typeof currentDestICAO !== 'undefined' ? currentDestICAO : '',
            tasKts: parseInt(document.getElementById('tasSlider')?.value || 115),
            profileCruiseFt: parseInt(document.getElementById('altMapInput')?.textContent || document.getElementById('altSlider')?.value || 4500),
            profileClimbFpm: typeof vpClimbRate === 'number' ? vpClimbRate : 500,
            profileDescentFpm: typeof vpDescentRate === 'number' ? vpDescentRate : 500
        }
    };
}
...
window.gaNavigationAvailable = () => window.liveTrackerConnected === true && !window.simModeActive
    && (window.liveTrackerCapabilities || []).includes('navigation.route.v1');
let navigationReadPromise = null;
let navigationReceived = null;
function navigationCanDisplay(nav) {
    if (nav.runId) return _missionExecutionAuthorityIsTracker() && nav.missionId === _activeMissionRuntimeId('');
    return !_activeMissionRuntimeId('');
}
function navigationRender(nav) {
    navigationReceived = nav;
    if (window.gaNavigationDragging || !navigationCanDisplay(nav)) return;
    window.gaApplyNavigationRoute?.(nav);
}
const navigationClient = window.GAMapNavigationClient.create({
    request: (intent, payload, revision) => navigationRequest(intent, payload, revision),
    render: navigationRender,
    error: () => alert('Die Route wurde inzwischen geändert oder die Verbindung ist unterbrochen. Bitte die aktuelle Route prüfen.'),
    refresh: () => navigationRead()
});
async function navigationRequest(intent, payload = {}, expectedRevision = 0) {
    if (!window.gaNavigationAvailable()) throw new Error('tracker_not_connected');
    return _sendMissionAuthorityRequest({ type: 'navigation_route_request', intent, payload, expectedRevision,
        clientId: _missionAuthorityClientId() }, 10000);
}
function navigationRead() {
    if (!window.gaNavigationAvailable()) return Promise.resolve(null);
    if (!navigationReadPromise) navigationReadPromise = navigationRequest('navigation_get').then(result => {
        if (result.ok && result.navigation && !window.gaNavigationDragging) navigationClient.receive(result.navigation);
        return result;
    }).catch(() => null).finally(() => { navigationReadPromise = null; });
    return navigationReadPromise;
}
window.gaNavigationRefresh = navigationRead;
window.gaNavigationReceive = nav => navigationClient.receive(nav);
window.gaNavigationEdit = function(edit) {
    if (!window.gaNavigationAvailable()) {
        // A previously shared route cannot silently become a local fork during a gap.
        if (navigationReceived?.points?.length && navigationCanDisplay(navigationReceived)) {
            alert('Tracker nicht verbunden. Bitte vor der Routenänderung die Verbindung wiederherstellen.');
            navigationRender(navigationReceived);
            if (typeof renderMainRoute === 'function') renderMainRoute();
            return true;
        }
        return false;
    }
    if (!navigationReceived || !navigationCanDisplay(navigationReceived)) {
        if (_missionExecutionAuthorityIsTracker()) { navigationRead(); return true; }
        return false; // Legacy/Web-authority keeps the existing App path.
    }
    if (window.missionComplianceBlockReset?.()) {
        if (typeof renderMainRoute === 'function') renderMainRoute();
        alert('Die laufende Behördenkontrolle muss zuerst abgeschlossen werden.'); return true;
    }
    if (navigationReceived.points.length < 2) {
        // Explicit editing of an existing private App route seeds the empty Tracker.
        const points = window.gaGetNavigationRoute?.();
        if (points?.length >= 2 && !_activeMissionRuntimeId('')) {
            navigationRequest('navigation_adopt', { routeId: navigationReceived.id, points,
                departureIcao: currentStartICAO, destinationIcao: currentDestICAO }, navigationReceived.revision)
                .then(result => { if (result.navigation) navigationClient.receive(result.navigation);
                    if (result.ok) navigationClient.edit(edit); else alert('Die Route hat sich geändert. Bitte erneut prüfen.'); })
                .catch(() => alert('Tracker nicht erreichbar.'));
        }
        return true;
    }
    if (!navigationClient.edit(edit) && typeof renderMainRoute === 'function') renderMainRoute();
    return true;
};
window.gaNavigationDirectTo = async function(airport, options) {
    await navigationRead();
    let nav = navigationClient.snapshot();
    if (nav && nav.points.length < 2 && !_activeMissionRuntimeId('') && window.gaGetNavigationRoute?.().length >= 2 && !options?.forceGpsStart) {
        const seed = await navigationRequest('navigation_adopt', { routeId: nav.id, points: window.gaGetNavigationRoute(),
            departureIcao: currentStartICAO, destinationIcao: currentDestICAO }, nav.revision);
        if (!seed.ok) { if (seed.navigation) navigationClient.receive(seed.navigation); return null; }
        nav = seed.navigation;
    }
    const result = await navigationRequest('airport_direct_to', { airport, forceGpsStart: !!options?.forceGpsStart }, nav?.revision || 0);
    if (!result.ok) {
        if (result.navigation) navigationClient.receive(result.navigation);
        alert('Direct To konnte nicht übernommen werden. Bitte den aktuellen Tracker-Routenstand prüfen.');
        return null;
    }
    return result.navigation;
};
window.addEventListener('gatrackercapabilitieschange', () => {
    if (!navigationReceived && window.gaNavigationAvailable()) navigationRead();
    else if (navigationReceived && navigationCanDisplay(navigationReceived)) navigationRender(navigationReceived);
});
...
function _applyTrackerPayloadControl(rawPayload = null) {
    if (!window.missionCargoStatus || !rawPayload || typeof rawPayload !== 'object') return false;
    const payload = window.GAMissionPayloadCore?.projectOutcome
        ? window.GAMissionPayloadCore.projectOutcome(rawPayload)
        : rawPayload;
    ...
    window.missionCargoStatus.payloadVerificationRunning = status === 'pending';
    window.missionCargoStatus.payloadStartOverride = payload.override === true;
    window.missionCargoStatus.payloadNeedsSync = status === 'warning' || status === 'error';
    window.missionCargoStatus.payloadPlan = payload.plan || null;
...
function _applyTrackerExecutionControl(control = null, activeRun = null, reason = 'tracker-execution') {
    ...
    const projectionSignature = JSON.stringify({
        ...
        manifest: control.manifest || null,
        cargo: control.cargo || null,
        payload: control.payload || null,
        voice: control.voice || null,
    ...
    if (trackerCompliance && typeof trackerCompliance === 'object') {
        try {
            if (typeof currentMissionData !== 'undefined' && currentMissionData && typeof currentMissionData === 'object') {
                currentMissionData.complianceInspection = _safeCloneJson(trackerCompliance, null);
            ...
            if (window.activeMissionContract && typeof window.activeMissionContract === 'object') {
                window.activeMissionContract.complianceInspection = _safeCloneJson(trackerCompliance, null);
            }
        } catch (_) {}
    }
    if (control.authoritySanction?.type === 'authority_sanction' && control.authoritySanction?.flightId) {
        try { window.addAuthoritySanctionToCrewboard?.(control.authoritySanction); } catch (_) {}
    }
    missionRuntime.phase = phase;
    missionRuntime.active = flags.active === true;
    missionRuntime.armed = flags.active === true;
    missionRuntime.manual = flags.started === true;
    missionRuntime.closingPending = flags.closingPending === true || phase === 'closing';
    if (flags.started === true && !missionRuntime.startedAt) missionRuntime.startedAt = Date.now();
    if (window.missionSceneStatus && typeof window.missionSceneStatus === 'object') {
        window.missionSceneStatus.boardingPreparing = phase === 'prepare';
        window.missionSceneStatus.boardingRequested = phase === 'boarding' && flags.boardingConfirmed !== true;
        window.missionSceneStatus.boardingActive = phase === 'boarding' && flags.boardingConfirmed !== true;
        window.missionSceneStatus.boardingComplete = flags.boardingConfirmed === true;
        window.missionSceneStatus.boardingVoiceComplete = flags.boardingConfirmed === true;
        window.missionSceneStatus.personBoarded = flags.boardingConfirmed === true;
    }
    const manifest = typeof _missionCargoEnsureManifest === 'function' ? _missionCargoEnsureManifest() : null;
    const trackerManifest = control.manifest && typeof control.manifest === 'object'
        && Array.isArray(control.manifest.items)
        ? control.manifest
        : null;
    const trackerCargo = control.cargo && typeof control.cargo === 'object' ? control.cargo : null;
    if (manifest && trackerManifest) {
        ...
        try {
            if (typeof currentMissionData !== 'undefined' && currentMissionData) currentMissionData.cargoManifest = manifest;
            if (window.activeMissionContract && typeof window.activeMissionContract === 'object') {
                window.activeMissionContract.cargoManifest = manifest;
            }
        } catch (_) {}
    } else if (manifest && trackerCargo && Array.isArray(trackerCargo.items)) {
        const byId = new Map(trackerCargo.items.map(item => [_trackerExecutionNormalizeItemId(item?.id), item]));
        (manifest.items || []).forEach(item => {
            const tracked = byId.get(_trackerExecutionNormalizeItemId(item?.id));
            if (!tracked) return;
            item.status = String(tracked.status || item.status || 'pending');
            if (Number.isFinite(Number(tracked.healthPct))) item.healthPct = Number(tracked.healthPct);
        });
        const signatureScope = String(trackerCargo.signatureScope || '').trim();
        manifest.dispatchSignature = signatureScope ? {
            by: 'Tracker',
            at: Number(control.updatedAt || Date.now()),
            aircraft: typeof _missionCargoAircraftLabel === 'function' ? _missionCargoAircraftLabel() : '',
    ...
    } else if (openBoardingDialog) {
        try { window.openMissionCargoDialog?.('load', { trackerProjection: true }); } catch (_) {}
    }
    _missionPhaseDebugPush('tracker_execution_projection', {
        reason,
        missionId,
        runId: control.runId || activeRun?.runId || null,
        phase,
        authorityRevision: Number(control.authorityRevision || 0),
        executionRevision: Number(control.executionRevision || 0),
        allowedActions: Array.isArray(control.allowedActions) ? control.allowedActions.slice() : []
    });
...
function _finalizeTrackerExecutionProjection(control = null, reason = 'tracker-execution-finalized') {
    if (!control || typeof control !== 'object' || control.executionAuthority !== 'tracker') return false;
    if (String(control.phase || '').toLowerCase() !== 'closed' || control.flags?.closed !== true) return false;
    const missionId = _normalizeMissionRuntimeId(control.missionId || '');
    if (!missionId || missionId !== _activeMissionRuntimeId('')) return false;
    const runId = String(control.runId || '').trim();
    ...
    missionRuntime.closingRequestedAt = Number(control.updatedAt || Date.now());
    let outcome = null;
    ...
    const record = _buildMissionCompletionRecord({
        ...
        outcome: missionRuntime.closingOutcome
    });
    if (record) {
...
async function _refreshTrackerExecutionControl(reason = 'intent-refresh') {
    ...
    try {
        ...
        if (activeRun) window.lastTrackerMissionStatus = { ...activeRun, receivedAt: Date.now() };
        if (!_applyTrackerExecutionControl(snapshot?.control, activeRun, reason)) {
            _finalizeTrackerExecutionProjection(snapshot?.lastControl, `${reason}:completed`);
        }
        return snapshot;
    } catch (_) {
        return null;
...
async function _pushMissionAuthoritySnapshotForExecutionHandoff(reason = 'execution-handoff-seed') {
    const local = _readMissionAuthorityState();
    const missionId = _activeMissionRuntimeId('');
    if (!local?.runId || !missionId || local.missionId !== missionId || local.clientId !== _missionAuthorityClientId()) {
...
async function _ensureTrackerExecutionAuthority(reason = 'apt-ui-intent') {
    ...
    missionExecutionHandoffPromise = (async () => {
        ...
        const committed = await _sendMissionAuthorityRequest({
            ...
            missionId: preparedRun.missionId,
            runId: preparedRun.runId,
            clientId: _missionAuthorityClientId(),
            expectedRevision: Number(preparedRun.revision),
            expectedExecutionStateHash: preparedRun.executionStateHash,
...
async function _submitTrackerExecutionIntent(intent, payload = {}, options = {}) {
    ...
    const execute = async () => {
        ...
        if (intent === 'request_close'
            ...
            try {
                const farewellRecord = missionRuntime.arrivalFlightRecord
                    || missionRuntime.pendingFarewellRecord
                    || _buildFlightRecordSnapshot(Date.now())
                    || null;
                const farewellVoiceRecipe = window.paxVoiceBuildFarewellEffectRecipe(farewellRecord);
                if (farewellVoiceRecipe) intentPayload.farewellVoiceRecipe = farewellVoiceRecipe;
            } catch (_) {}
        }
        ...
        if (window.liveTrackerConnected && typeof window.sendTrackerCommand === 'function') {
            ...
            const stillAllowed = latestControl?.executionAuthority === 'tracker'
                ...
                && (allowedActions.includes(intent) || ['close_cargo_window', 'open_cargo_window'].includes(intent));
            if (retryable
...
function _applyTrackerExecutionAbortLocally(snapshot = null, reason = 'tracker-execution-aborted', options = {}) {
    ...
    const resetOk = window.missionRuntimeReset?.({
        ...
        skipAuthorityRelease: true,
        complianceReleased: true,
        forceComplianceReset: true,
    ...
    if (resetOk !== false) window.gaTrackerExecutionAbortedRunId = abortedRun.runId;
    _missionPhaseDebugPush('tracker_execution_aborted', {
        reason,
        missionId,
...
window.gaAbortTrackerMission = async function(options = {}) {
    if (missionExecutionAbortPromise) {
        const pendingResult = await missionExecutionAbortPromise;
        if (pendingResult?.ok === true && options.forceLocalCleanup === true) {
            const localCleared = _applyTrackerExecutionAbortLocally(
                { lastRun: pendingResult.releasedRun },
                options.reason || 'app-tracker-abort',
            ...
            if (!localCleared) {
                ...
                return { ...pendingResult, ok: false, status: 'error', error: 'mission_local_cleanup_failed' };
            }
        }
    ...
    missionExecutionAbortPromise = (async () => {
        ...
        if (missionExecutionIntentPromise) {
            try { await missionExecutionIntentPromise; } catch (_) {}
        }
        const reason = String(options.reason || 'app-tracker-abort');
        const result = await window.gaTrackerExecutionSubmitIntent?.('abort_mission', { reason }, { silent: true });
        if (result?.ok === true) {
            const localCleared = _applyTrackerExecutionAbortLocally(
            ...
            if (options.forceLocalCleanup === true && !localCleared) {
                try { alert('Die Tracker-Mission wurde beendet, aber der lokale App-Stand konnte nicht vollständig geleert werden. Bitte Clear erneut ausführen.'); } catch (_) {}
                return { ...result, ok: false, status: 'error', error: 'mission_local_cleanup_failed' };
            }
            return result;
        }
        const message = result?.error === 'mission_revision_conflict'
            ? 'Eine andere Ansicht hat den Missionsstand bereits geändert. Bitte den aktualisierten Stand erneut prüfen.'
            : `Tracker-Mission konnte nicht abgebrochen werden: ${result?.error || result?.status || 'unbekannter Fehler'}`;
        try { alert(message); } catch (_) {}
        return result || { ok: false, status: 'error', error: 'mission_abort_failed' };
    })();
    try {
        return await missionExecutionAbortPromise;
    } finally {
        missionExecutionAbortPromise = null;
    }
};
...
window.gaPushMissionAuthorityRoute = function(reason = 'route-changed') {
    if (window.gaNavigationAvailable() && navigationReceived && navigationCanDisplay(navigationReceived)) return true;
...
function _buildMissionRuntimeSnapshot(reason = 'runtime') {
    ...
    const cargoManifest = (typeof _missionCargoGetManifest === 'function')
        ...
        : null;
    const payloadStatus = window.missionCargoStatus && typeof window.missionCargoStatus === 'object'
        ? window.missionCargoStatus
        : {};
    const payloadVerification = payloadStatus.payloadVerification && typeof payloadStatus.payloadVerification === 'object'
        ? payloadStatus.payloadVerification
        : null;
    const payloadOutcomeStatus = payloadStatus.payloadSyncRunning || payloadStatus.payloadVerificationRunning
        ? 'pending'
        : (payloadVerification?.status === 'ok'
            ? 'ok'
            : (payloadVerification?.status === 'unstable' || (payloadStatus.payloadNeedsSync && payloadStatus.error)
                ? 'warning'
                : 'idle'));
    const missionCargoPayloadOutcome = window.GAMissionPayloadCore?.normalizeOutcome
        ? window.GAMissionPayloadCore.normalizeOutcome({
            status: payloadOutcomeStatus,
            override: payloadStatus.payloadStartOverride === true,
            adapter: payloadStatus.payloadPlan?.payloadAdapter || payloadStatus.payloadBaseline?.payloadAdapter || null,
            error: payloadStatus.error || null,
            plan: payloadStatus.payloadPlan || null,
            verification: payloadVerification
        }, { updatedAt: Number(payloadStatus.payloadSyncAt || 0) })
        : null;
    return {
        version: 1,
        missionId,
...
function _handleTrackerMissionAuthoritySnapshot(snapshot = null, reason = 'tracker-authority') {
    ...
    window.lastTrackerMissionAuthority = { ...snapshot, receivedAt: Date.now() };
    if (snapshot.execution?.executionAuthority === 'tracker') {
        _applyTrackerExecutionControl(snapshot.execution, active, reason);
    }
    if (relation === 'demote') {
...
window.resumeTrackerMissionOnThisDevice = async function(options = {}) {
    ...
    if (authoritativeRun.executionAuthority === 'tracker') {
        // Bei Tracker-Execution gibt es keinen Browser-Owner mehr zu uebernehmen.
        // Die App restauriert das private Bundle nur als Beobachter und bezieht
        // Phase, Manifest und erlaubte Aktionen anschliessend wieder vom Tracker.
        ...
        let restoredObserver = false;
        try {
            restoredObserver = (await restoreMissionState(bundle.missionState, {
                source: 'tracker-execution-observer',
                allowDraft: false,
                resumeRuntime: true,
                authorityConfirmed: true
            })) !== false;
        } catch (error) {
            console.warn('[MISSION AUTHORITY] Tracker-Execution konnte lokal nicht als Beobachter restauriert werden:', error);
        }
        if (!restoredObserver) return false;
        _restoreMissionAuthorityMapProfile(bundle);
        if (missionRuntimeResumeAppliedFor !== _normalizeMissionRuntimeId(authoritativeRun.missionId)) {
            _restoreMissionRuntimeFromSnapshot(bundle.runtime, {
                reason: 'tracker-execution-observer',
                trackerConfirmed: true,
                trackerActive: true,
                authorityConfirmed: true
            });
        ...
        _missionPhaseDebugPush('tracker_execution_observer_restored', {
            ...
            revision: Number(authoritativeRun.revision || 0),
...
window.confirmMissionAuthorityReplacement = function(label = 'diese Aktion', reason = 'mission-replacement') {
    ...
    if (window.missionRuntimeResumeConflict?.trackerActive === true) {
        if (_missionExecutionAuthorityIsTracker()) {
            let confirmed = false;
            try {
                confirmed = confirm(`Auf dem Tracker läuft noch Mission ${window.missionRuntimeResumeConflict?.trackerMissionId || ''}.\n\nDiese Mission auf allen Ansichten abbrechen und danach ${label} erneut starten?`);
            } catch (_) {}
            if (confirmed) {
                window.gaAbortTrackerMission?.({ skipConfirm: true, reason }).then(result => {
                    if (result?.ok === true) {
                        try { alert(`Die Tracker-Mission wurde abgebrochen. Du kannst ${label} jetzt erneut starten.`); } catch (_) {}
                    }
                });
            }
            return false;
    ...
    const reset = window.missionRuntimeReset?.({ respawnAfterClear: false, authorityOutcome: 'aborted', reason });
    return reset !== false;
};
...
function _missionSceneBuildDeboardingEffectCommand(reason = 'mission-end', position = {}, sceneId = _missionSceneId()) {
    ...
    const deboardingPaxCount = Math.max(
        _missionSceneDeboardingPaxCount(),
...
function _buildMissionAptExecutionEffectPlan() {
    ...
    try {
        boardingVoice = typeof window.paxVoiceBuildBoardingEffectRecipe === 'function'
            ...
            : null;
        if (boardingVoice && typeof boardingVoice === 'object') boardingVoice.missionId = missionId;
    } catch (_) {
        boardingVoice = null;
    }
    try {
        ...
        farewellContext = typeof window.paxVoiceBuildFarewellAuthorityContext === 'function'
            ...
            : null;
        farewellVoice = typeof window.paxVoiceBuildFarewellEffectRecipe === 'function'
            ? window.paxVoiceBuildFarewellEffectRecipe(null)
            : null;
        if (farewellVoice && typeof farewellVoice === 'object') farewellVoice.missionId = missionId;
    } catch (_) {
        farewellContext = null;
        farewellVoice = null;
    }
...
function _missionComplianceBuildGroundVisitCommand(state = null, reason = 'authority-inspection', pos = {}) {
...
window.missionComplianceBuildGroundVisitEffectCommand = function(reason = 'tracker-execution:scene.compliance_visit') {
    return _missionComplianceBuildGroundVisitCommand(null, reason, { lat: 0, lon: 0, alt: 0, hdg: 0 });
};
...
window.missionComplianceStartGroundVisit = function(state = null, reason = 'authority-inspection') {
    ...
    const command = _missionComplianceBuildGroundVisitCommand(state, reason, pos);
    const commandId = window.sendTrackerCommand(command);
    if (!commandId) return false;
    return { commandId, sceneId: command.sceneId };
};
...
function _handleTrackerAck(ack) {
    ...
    if (ackType === 'navigation_route_ack') {
        _trackerPendingHandleAck(ack);
        _resolveMissionAuthorityAck(ack);
        return;
    }
    if (ackType === 'navigation_route_changed') {
        if (window.gaNavigationAvailable() && !window.gaNavigationDragging) navigationClient.receive(ack.navigation);
        return;
    }
    ...
    _trackerPendingHandleAck(ack);
    const authorityAck = /^mission_(authority|snapshot|execution_authority)_.*_ack$/i.test(String(ack.type || ''));
    if (authorityAck) {
        _resolveMissionAuthorityAck(ack);
...
function _missionCargoMaybePromoteStartReady(reason = 'cargo-ready-check') {
    const manifest = _missionCargoEnsureManifest();
    const startPolicy = window.GAMissionStartCore;
    const readiness = startPolicy?.deriveStartReadiness?.({
        loadConfirmed: !!window.missionCargoStatus?.loadConfirmed,
        dispatchSigned: !!manifest?.dispatchSignature,
        loadInteractionReady: _missionCargoLoadInteractionReady(),
        boardingVoiceComplete: !!window.missionSceneStatus?.boardingVoiceComplete,
        alreadyBoarded: _missionStartPhase() === 'boarded'
    });
    if (readiness ? !readiness.ready : (
        !window.missionCargoStatus?.loadConfirmed
        || !manifest?.dispatchSignature
        || !_missionCargoLoadInteractionReady()
        || !window.missionSceneStatus?.boardingVoiceComplete
    )) return false;
    if (readiness?.alreadyBoarded || _missionStartPhase() === 'boarded') return true;
...
function _buildMissionCompletionRecord(options = {}) {
    ...
    const flight = options.flightRecord
        || window.gaTrackerExecutionControl?.flight?.missionRecord
        || missionRuntime.pendingFarewellRecord
    ...
    return {
        ...
        cruiseDurationSec,
        segmentCount: Math.max(1, Math.round(Number(flight.segmentCount) || 1)),
        comfort: comfort ? {
...
function _missionStartBannerDismissed(actionKey = 'legacy') {
    try {
        const key = _missionStartUiKey();
        return !!key && missionStartBannerDismissState[key] === String(actionKey || 'legacy');
    } catch (_) {
        return false;
    }
}
...
window.dismissMissionStartBanner = function() {
    try {
        ...
        if (key) missionStartBannerDismissState[key] = String(banner?._gaMissionActionKey || 'legacy');
    } catch (_) {}
    _updateMissionRuntimeUi();
};
...
function _missionCriticalActionConfirmMessage(action = 'end', options = {}) {
    ...
    if (normalized === 'abort') {
        return 'Tracker-Mission wirklich abbrechen?\n\nDie Mission wird auf allen verbundenen Ansichten beendet, ihre Sim-Objekte werden entfernt und der Flug wird nicht als abgeschlossen gewertet.';
    }
    if (normalized === 'stop') {
...
function _trackerMissionBannerModel(control = null) {
    ...
    const labels = {
        ...
        request_close: 'Alle Abschlussbedingungen sind erfüllt.'
    };
    let model = null;
    if (actions.includes('activate_cloud_mission')) {
        ...
        model = { intent: 'prepare_mission', kind: 'intent', kicker: 'Mission bereit', text: labels.prepare_mission, button: 'Mission beginnen', className: 'is-begin-action' };
    } else if (actions.includes('start_boarding')) {
        model = { intent: 'start_boarding', kind: 'intent', kicker: 'Mission bereit', text: labels.start_boarding, button: 'Boarding und Verladen beginnen', className: '' };
        ...
        const arrivalPax = /^(end_unloading|end_ready)$/.test(phase)
            && actions.includes('request_pax_interaction')
            && manifestItems.some(item => item?.itemType === 'passenger'
                && item?.status === 'loaded'
                && String(item?.delivery || 'destination') === 'destination');
        if (cargoAction || arrivalPax) {
            const arrival = /^(end_unloading|end_ready)$/.test(phase);
            model = {
                intent: '',
                kind: 'cargo',
                cargoMode: arrival ? 'unload' : (phase === 'on_task' ? 'pickup' : 'load'),
                kicker: arrival ? 'Ankunftsaktion bereit' : (phase === 'on_task' ? 'Aktion am Ziel' : 'Boarding und Verladung'),
                text: arrival
                    ? 'Der Tracker hat Entladung und Passagierausstieg freigegeben.'
                    : 'Der Tracker hat die nächste Verladeaktion freigegeben.',
    ...
    model.key = [control.missionId || '', Number(control.authorityRevision || 0), phase, model.kind, model.intent].join(':');
    return model;
}

function _renderTrackerMissionBanner(banner, control, elements = {}) {
    const model = _trackerMissionBannerModel(control);
    banner._gaTrackerAction = model;
    banner._gaMissionActionKey = model?.key || '';
    banner.classList.toggle('is-end-ready', model?.endReady === true || model?.className === 'is-end-ready');
    banner.classList.toggle('is-begin-action', model?.begin === true || model?.className === 'is-begin-action');
    banner.classList.toggle('is-final-action', model?.final === true || model?.className === 'is-final-action');
    if (!model || _missionStartBannerDismissed(model.key)) {
        banner.style.display = 'none';
        return true;
    }
    banner.style.display = 'flex';
    if (elements.kicker) elements.kicker.textContent = model.kicker;
    if (elements.text) elements.text.textContent = model.text;
    if (elements.close) elements.close.style.display = model.closeHidden === true ? 'none' : '';
    if (elements.button) {
        elements.button.textContent = model.button;
        elements.button.disabled = !!missionExecutionIntentPromise || model.disabled === true;
    }
    return true;
}
...
function _updateMissionStartBanner() {
    ...
    if (!authorityConflict && trackerControl?.executionAuthority === 'tracker') {
        const complianceUi = window.missionComplianceGetCargoUiState?.() || null;
        if (complianceUi?.active === true
            && !['selected', 'not_selected', 'none', 'released'].includes(String(complianceUi.phase || ''))) {
            banner.style.display = 'none';
            try { window.missionComplianceRefreshUi?.(); } catch (_) {}
            return;
        }
        _renderTrackerMissionBanner(banner, trackerControl, {
            kicker: kickerEl,
            text: textEl,
            button: btn,
            close: closeBtn
    ...
    banner._gaTrackerAction = null;
    banner._gaMissionActionKey = 'legacy';
...
window.missionRuntimeReset = function(options = {}) {
    if (_missionExecutionAuthorityIsTracker() && options?.trackerAbortCompleted !== true) {
        _missionPhaseDebugPush('runtime_reset_blocked', {
            ...
            block: 'tracker-execution-authority'
        });
        try { alert('Diese Mission läuft autoritativ im Tracker. Nutze „Mission abbrechen“, damit alle verbundenen Ansichten gemeinsam bereinigt werden.'); } catch (_) {}
        return false;
    }
    ...
    if (options?.trackerAbortCompleted !== true
        && _trackerSupportsMissionAuthority()
    ...
    if (!skipAuthorityRelease && !authorityReleaseRequested) _sendMissionLifecycleToTracker('ended', resetReason);
...
window.manualMissionEnd = function(options = {}) {
    if (_missionExecutionAuthorityIsTracker()) {
        return window.gaTrackerExecutionSubmitIntent?.('request_close') || false;
    }
    _missionPhaseDebugPush('trigger', {
...
window.handleMissionStartBannerAction = async function() {
    ...
    missionStartActionPromise = (async () => {
        ...
        if (_missionExecutionAuthorityIsTracker() && trackerBannerAction) {
            if (trackerBannerAction.kind === 'cargo') {
                window.openMissionCargoDialog?.(trackerBannerAction.cargoMode || 'load');
                return true;
            }
        ...
        if (missionRuntime.closingPending) {
            if (_missionExecutionAuthorityIsTracker()) {
                await _refreshTrackerExecutionControl('banner-closing-refresh');
                return true;
            }
        ...
        if (missionRuntime.active) {
            ...
            if (_missionExecutionAuthorityIsTracker()) {
                await window.gaTrackerExecutionSubmitIntent?.('request_close');
                return;
            }
        ...
        if (phase === 'prepare') {
            if (_missionExecutionAuthorityIsTracker()) {
                await _refreshTrackerExecutionControl('banner-prepare-refresh');
                return true;
            }
        ...
        if (_missionExecutionAuthorityIsTracker()) {
            const result = await window.gaTrackerExecutionSubmitIntent?.('start_mission');
            return result?.ok === true;
        }
...
function _syncTrackerMissionSeedPayload(activeMission = null) {
    ...
    const missionId = _activeMissionRuntimeId('');
    if (!missionId || _missionStartPhase() !== 'planned' || missionRuntime.active || missionRuntime.closingPending) return null;
    const runtime = {
        version: 1,
        missionId,
        startedAt: 0,
    ...
    try {
        efbMission = typeof window.gaGetEfbMissionViewSnapshot === 'function'
            ? _safeCloneJson(window.gaGetEfbMissionViewSnapshot(), null)
            : null;
        if (efbMission && typeof efbMission === 'object') delete efbMission.capturedAt;
    } catch (_) {
        efbMission = null;
    }
    const md = state.currentMissionData && typeof state.currentMissionData === 'object'
    ...
    const contract = state.activeMissionContract && typeof state.activeMissionContract === 'object'
        ? state.activeMissionContract
        : (md.missionContract && typeof md.missionContract === 'object' ? md.missionContract : null);
    const initialCargoManifest = _safeCloneJson(
        state.cargoManifest || md.cargoManifest || contract?.cargoManifest || null,
        null
    );
    const updatedAt = Math.max(
        0,
        Number(state.activeMissionSavedAt || 0),
        Number(md.activeMissionSavedAt || 0),
        Number(state.activeMissionCreatedAt || 0),
        Number(md.activeMissionCreatedAt || 0)
    );
    return {
        schema: 'ga.tracker-cloud-mission-seed.v1',
        version: 1,
        missionId,
        adapter,
        updatedAt,
        descriptor: typeof window.GAMissionResumeAdapters?.createDescriptor === 'function'
            ? window.GAMissionResumeAdapters.createDescriptor(runtime, state)
            : null,
...
async function _syncApplyActiveMissionFromCloud(activeMission = null, options = {}) {
    ...
    if (trackerRun) {
        ...
        _syncRecordCloudMissionPullOutcome(
            resumed
                ? (trackerExecutionObserver ? 'tracker-observer-restored' : 'tracker-handoff-complete')
                : (trackerExecutionObserver ? 'tracker-observer-incomplete' : 'tracker-handoff-incomplete'),
            commonOutcome
        );
        return resumed;
    }
...
function _markTrackerHeartbeat(pkt) {
    ...
    if (pkt?.trackerNavigation && window.gaNavigationAvailable()) {
        const nav = navigationClient.snapshot();
        if (!nav || nav.id !== pkt.trackerNavigation.id || nav.revision !== pkt.trackerNavigation.revision) navigationRead();
    }
...
const liveCompassPresentation = window.GAMapLivePresentation.createCompass(document);
function buildCompassSvg() { liveCompassPresentation.buildRose(); }
function buildCompassFixed() { liveCompassPresentation.buildFixed(); }
window.updateCompassInstruments = function(bearingToWp, courseDeg, xteNm) {
    liveCompassPresentation.updateInstruments(bearingToWp, courseDeg, xteNm);
};
...
window.updateCompassHeading = function(hdg) {
    ...
    liveCompassPresentation.updateHeading(hdg);
...
function updateLiveToActiveWpLine(lat, lon, activeWpIdx = null) {
    ...
    if (!liveToWpLine) {
        liveToWpLine = L.polyline(pts, window.GAMapLivePresentation.DIRECT_LINE_STYLE).addTo(map);
    } else {
...
function updateNextWpTelemetry(lat, lon) {
    ...
    const selection = window.GAMapNavigationGeometry.selectWaypoint({lat, lon}, routeWaypoints,
        {routeKey:liveNextRouteKey, legIndex:liveNextLegIndex, selectedIndex:liveActiveWpIndex});
    if (!selection) return;
    liveNextRouteKey = selection.routeKey;
    liveNextLegIndex = selection.legIndex;
    liveActiveWpIndex = selection.selectedIndex;
    const wpIdx = selection.wpIdx, maxWp = routeWaypoints.length - 1;
    if (selection.automaticAdvance && typeof window.awmAnnounceWpAdvance === 'function') {
        window.awmAnnounceWpAdvance(selection.brng, selection.dist);
    }
    ...
    if (typeof window.updateCompassInstruments === 'function') {
        window.updateCompassInstruments(nav.brng, selection.inboundBrng, selection.xteNm);
    }
    return nextInfo;
}
        ...
        try {
            ...
            if (data.type === 'gps') {
                ...
                if (data.traffic && Array.isArray(data.traffic)) {
                    ...
                    const filteredTraffic = window.GAMapLivePresentation.filterTraffic(data.traffic, data);
...
function _recordLiveTrailPoint(lat, lon) {
    const next = window.GAMapLivePresentation.appendTrailPoint(liveSnailTrailPoints, lat, lon, _liveTrailDistanceM);
    if (!next) return false;
    if (next !== liveSnailTrailPoints) liveSnailTrailNeedsFullSync = true;
    liveSnailTrailPoints = next;
    lastTrailPoint = next[next.length - 1];
...
function updateLivePlanePosition(lat, lon, alt, hdg) {
    ...
    if (liveMapVisualActive) updateAutoFollowFromTelemetry({ lat, lon, alt, gs: autoFollowGs, hdg, now, lowFpsMode });
    ...
    if (smoothedGS > 30 && typeof getDestinationPoint === 'function' && now - lastPredictionUpdate > 1000) {
        ...
        const predPoints = window.GAMapPrediction.points({lat, lon, alt, hdg}, smoothedGS, smoothedVS);
        ...
        if (liveMapVisualActive) {
            if (!livePredictionLayer) livePredictionLayer = window.GAMapPrediction.createLayer(L, map);
            livePredictionLayer.render({lat, lon}, predPoints);
        }
        ...
        if (typeof checkTerrainAlongPath === 'function') {
            ...
            checkTerrainAlongPath(_tawsPredPoints).then(results => {
                ...
                window.GAMapPrediction.applyTerrain(predPoints, results, _getAirspaceColorForPredPoint);
                if (_canRunLiveMapVisualWork() && livePredictionLayer) livePredictionLayer.colorize(predPoints, _getAirspaceColorForPredPoint);
            });
        } else {
        ...
        if (livePredictionLayer) livePredictionLayer.clear();
    ...
    if (liveMapVisualActive && !liveGpsMarker) {
        const _planeSvgTemplate = window.GAMapLivePresentation.aircraftHtml();
    ...
    vpUpdateLiveProfilePosition(lat, lon, alt, hdg, liveMapVisualActive);
...
function _buildFlightRecordSnapshot(now) {
    ...
    const sharedRecorder = window.GAMissionFlightRecorderCore;
    if (sharedRecorder && typeof sharedRecorder.buildRecord === 'function') {
        const sharedRecord = sharedRecorder.buildRecord(r, {
            now: endTs,
            depLabel,
            arrLabel
        });
