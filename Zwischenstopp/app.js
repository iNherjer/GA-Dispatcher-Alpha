...
function renderMainRoute() {
    ...
    routeWaypoints.forEach((latlng, index) => {
        ...
        if (draggable) {
            // Beim Ziehen: Magnet-Prüfung
            marker.on('drag', function(e) {
                if (snapMode && cachedNavData.length > 0) {
                    let mousePoint = map.latLngToLayerPoint(e.latlng);
                    let closest = null;
                    let minDist = 25; // Einrast-Radius in Pixeln
                    
                    cachedNavData.forEach(nav => {
                        let navPoint = map.latLngToLayerPoint([nav.lat, nav.lng]);
                        if (mousePoint.distanceTo(navPoint) < minDist) {
                            minDist = mousePoint.distanceTo(navPoint);
                            closest = nav;
                        }
                    });
                    
                    if (closest) {
                        marker.setLatLng([closest.lat, closest.lng]); // Snapping!
                        marker._snappedData = closest;
                    } else {
                        marker.setLatLng(e.latlng); // Folgt normal der Maus
                        marker._snappedData = null;
                    }
                }
            });

            // Beim Loslassen: Koordinaten und Namen speichern
            marker.on('dragend', function(e) {
                let finalLatLng = marker.getLatLng();
                routeWaypoints[index].lat = finalLatLng.lat;
                routeWaypoints[index].lng = finalLatLng.lng;
                
                if (marker._snappedData) {
                    routeWaypoints[index].name = marker._snappedData.name;
                } else {
...
function updateRoutePerformance() {
    ...
    for(let i=0; i<routeWaypoints.length - 1; i++) {
        ...
        totalNM += nav.dist;
        let name1 = (i === 0) ? currentSName : (routeWaypoints[i].name || `WP ${i}`);
        let name2 = (i === routeWaypoints.length - 2) ? currentDName : (routeWaypoints[i+1].name || `WP ${i+1}`);
...
async function fetchOpenAIPData() {
    ...
    try {
        // Der Cache-Buster: &t=${Date.now()} zwingt den Browser zum Neuladen!
        const [navRes, repRes] = await Promise.all([
        ...
        console.log("🔍 VOR/NDB:", navJson);
        console.log("🔍 RPP:", repJson);
        
        cachedNavData = [];
        
        let navArray = navJson.items || navJson.data || navJson.results || (Array.isArray(navJson) ? navJson : []);
        let repArray = repJson.items || repJson.data || repJson.results || (Array.isArray(repJson) ? repJson : []);

        navArray.forEach(i => {
            if (!i.geometry || !i.geometry.coordinates) return;
            let freq = (i.frequencies && i.frequencies.length > 0) ? ` (${i.frequencies[0].value})` : '';
            cachedNavData.push({ name: `${i.name}${freq}`, lat: i.geometry.coordinates[1], lng: i.geometry.coordinates[0] });
        });
        ...
        console.log(`🧲 Magnet-Punkte geladen: ${cachedNavData.length} Stück.`);
