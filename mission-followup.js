(function() {
    ...
    function buildNarrativeMemory(sourceKind, md, passenger, homeRef, targetRef) {
        ...
        if (sourceKind === 'bush_charter_strip') {
            ...
            return {
                ...
                stayOrWorkSummary: `${name} hat am Zielstrip den geplanten Aufenthalt genutzt: Briefing mit dem lokalen Kontakt, Kontrolle der mitgebrachten Ausruestung und Abgleich der Lage vor Ort.`,
                whyNowReturn: 'Die Arbeit vor Ort ist abgeschlossen, die Notizen und persoenlichen Sachen sind gepackt, und die Basis braucht den Rueckbericht.',
                returnReason: 'Rueckkehr zur Basis fuer Debriefing und naechste Teamentscheidung.',
                ...
                pickupGreetingText: `${name || 'Ich'} bin bereit am Strip. Wir haben hier draussen alles erledigt und muessen mit dem Bericht zurueck zur Basis.`,
                sourcePaxText: pax,
        ...
        return {
            ...
            stayOrWorkSummary: `Die Crew vor Ort hat die Lieferung sortiert, Verbrauchsmaterial verteilt und die Rueckfracht fuer den Heimflug vorbereitet.`,
            whyNowReturn: 'Der Platzkontakt hat die Rueckholfracht freigegeben; sie soll nicht laenger am Strip liegen bleiben.',
            returnReason: 'Ruecktransport von Belegen, leeren Behaeltern und einem kleinen defekten Teil zur Basis.',
            teamContinuity: `Der Folgeflug schliesst den Versorgungskreislauf ab: hinbringen, vor Ort nutzbar machen, Rueckfracht sauber heimholen.`,
            pickupGreetingText: '',
            sourcePaxText: pax,
            sourceCargoText: cargo
        };
    ...
    function buildCargoReturn(memory, targetRef) {
        ...
        return {
            label: `Rueckholfracht ${place}: leere Versorgungskisten, signierte Materialliste und defektes Funkakku-Case (42 lbs)`,
            role: 'Frachtkontakt am Strip',
            weightLbs: 42,
            reason: memory?.returnReason || 'Rueckfracht zur Basis'
        };
    }
        ...
        const req = {
            ...
            ui: {
                ...
                previewText: sourceKind === 'bush_charter_strip'
                    ? `${passenger?.name || 'Der Gast'} meldet sich vom Strip zur Rueckholung.`
                    : `Am Strip wartet Rueckfracht aus dem Supply Run.`
            }
        };
    ...
    function render() {
        ...
        laterBtn.textContent = 'Spaeter';
        laterBtn.title = 'Hinweis fuer heute ausblenden, Anfrage bleibt in der Liste';
    ...
    function renderRequestItem(req) {
        ...
        dismiss.textContent = '×';
        dismiss.title = 'Anfrage loeschen';
    ...
    async function acceptRequest(id) {
        ...
        if (!duePendingRequests().some(item => item.id === id)) {
            alert('Diese Folgeanfrage ist noch nicht faellig.');
            return false;
        }
        ...
        if (!home?.icao || !target?.icao) {
            alert('Folgeanfrage unvollstaendig: Start oder Ziel fehlt.');
            return false;
        }
    ...
    function buildPickupStory(req, targetName, homeName) {
        ...
        return [
            ...
            `${memory.stayOrWorkSummary || 'Der Aufenthalt vor Ort ist abgeschlossen, die Ausruestung ist geordnet und der lokale Kontakt hat den Rueckflug freigegeben.'}`,
            `${memory.whyNowReturn || 'Die Basis braucht den Rueckbericht noch heute, bevor das Team die naechsten Schritte plant.'}`,
            `Du startest wieder in ${homeName}, fliegst leer zum bekannten Strip, nimmst ${name} am vereinbarten Treffpunkt auf und bringst ihn mit Notizen, Gepaeck und Lagebild zurueck zur Basis.`,
            `${memory.teamContinuity || 'Dass du den ersten Leg geflogen bist, macht den Rueckflug einfacher: Person, Anflug und Absprachen sind bereits vertraut.'}`
        ].filter(Boolean).join(' ');
    }

    function buildCargoStory(req, targetName, homeName) {
        const memory = req.narrativeMemory || {};
        const cargo = req.cargoReturn?.label || `Rueckholfracht ${targetName}`;
        return [
            `Folgeauftrag zum letzten Bush Supply Run: Am Strip bei ${targetName} wartet jetzt die Rueckfracht aus der Versorgungslieferung.`,
            `${memory.stayOrWorkSummary || 'Die Crew vor Ort hat die Lieferung verteilt, Material geprueft und die Ruecksendung bereitgelegt.'}`,
            `${memory.whyNowReturn || 'Der Platzkontakt moechte die Fracht nicht laenger am Rand des Strips stehen lassen.'}`,
            `Du startest wieder in ${homeName}, fliegst leer zum bekannten Zielstrip, uebernimmst ${cargo} und bringst alles zurueck zur Basis.`,
            `${memory.teamContinuity || 'Der Flug schliesst den Versorgungskreislauf sauber ab und gibt dem Team wieder klares Material- und Lagebild.'}`
        ].filter(Boolean).join(' ');
    }
    ...
    function buildDebugReport() {
        ...
        lines.push(`- Pending: ${pending.length} | faellig: ${due.length} | geplant: ${future.length}`);
        if (!list.length) {
    ...
    function updateDebugButton() {
        ...
        btn.title = future.length
            ? 'Naechste geplante Folgeanfrage sofort faellig machen'
            : (pending ? 'Es gibt bereits eine faellige Folgeanfrage' : 'Keine geplante Folgeanfrage vorhanden');
    }
    ...
    window.missionFollowupDebugForceNext = function() {
        ...
        render();
        alert('Naechste Follow-up-Anfrage ist jetzt verfuegbar.');
        return true;
    };
